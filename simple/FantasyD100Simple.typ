#import "@preview/owlbear:0.0.1": dnd-note, dnd-terms, dnd-enum

#let simple-theme = (
  paint: (
    chapter: (
      number: (fill: rgb(235, 214, 193)),
      heading: (
        fill: gradient.linear(rgb(121, 59, 47), rgb(181, 79, 66), angle: 90deg),
        stroke: rgb(121, 59, 47),
      ),
    ),
    dialogue: (fill: rgb(244, 232, 209)),
    dropcap: (
      fill: gradient.linear(rgb(209, 163, 71), rgb(234, 200, 137), angle: 90deg),
      stroke: rgb(209, 163, 71),
    ),
    heading: (
      fill: rgb(121, 59, 47),
      line: rgb(209, 163, 71),
    ...
    label: (
      dm: (fill: rgb(121, 59, 47)),
      player: (fill: rgb(71, 114, 128)),
    ),
    table: (
      even: rgb(215, 232, 238),
      odd: rgb(241, 247, 249),
    ),
  ),
  font: (
...
#let owlbear-rpts-template(body) = {
  set page(paper: "a4", margin: 1.7cm)
  set par(justify: true)
  set text(font: simple-theme.font.body, size: 11pt)
  show heading: set text(font: simple-theme.font.heading, fill: simple-theme.paint.heading.fill, weight: 400)
  show heading.where(level: 1): set text(
    size: 34pt,
    fill: simple-theme.paint.chapter.heading.fill,
    stroke: (paint: simple-theme.paint.chapter.heading.stroke, thickness: 1pt),
    weight: 300,
  )
  show heading.where(level: 2): set text(size: 18pt)
  show heading.where(level: 3): set text(size: 14pt)
  set table(
    fill: (x, y) => if y == 0 { none } else if calc.rem(y, 2) == 0 { simple-theme.paint.table.even } else { simple-theme.paint.table.odd },
    stroke: none,
  )
  show table: set text(font: simple-theme.font.table)
  show table.cell: it => if it.y == 0 { strong(it) } else { it }
  show figure.where(kind: table): it => {
    show figure.caption: set text(font: simple-theme.font.table, size: 14pt, weight: 500)
...
+ State the character's intent and stakes.
+ Choose the most relevant skill.
+ Set difficulty as bonus or penalty dice.
+ Roll D100, applying bonus or penalty dice if present.
+ Compare the roll to the skill and identify failure, success, critical, or fumble.
+ For opposed actions, roll for both sides and use the opposed test rules.
+ Apply consequences in the fiction first, then update numbers such as HP, Hero Points, equipment, time, position, or conditions.
...
#figure(
  caption: "Bonus and Penalty Dice",
  table(
    columns: (auto, 1fr),
    table.header([Modifier], [Procedure]),
...
#dnd-note(theme: simple-theme)[
  = When to Roll

  Use these rules whenever the fiction has uncertainty and meaningful consequences. Do not roll for trivial actions, tasks with no pressure, or outcomes that are already certain from the fiction.
]
...
#figure(
  ...
  table(
    columns: (1.5fr, auto),
    table.header([Situation], [Modifier]),
...
#figure(
  caption: "Opposed Tests",
  table(
    columns: (1fr, 1.4fr),
    table.header([Result], [Winner]),
...
#figure(
  ...
  table(
    columns: (auto, 1fr),
    table.header([Characteristic], [Meaning]),
...
#figure(
  ...
  table(
    columns: (auto, 1fr),
    table.header([Attribute], [Formula]),
...
#figure(
  ...
  table(
    columns: (auto, auto),
    table.header([STR + SIZ], [Damage Modifier]),
    [1-10], [-1D6],
    [11-15], [-1D4],
    [16-25], [+0],
    [26-30], [+1D4],
...
#figure(
  caption: "Skill Creation Points",
  table(
    columns: (auto, 1fr),
    table.header([Group], [Creation Points]),
...
#figure(
  ...
  text(size: 7.5pt)[
    #table(
      columns: (1.1fr, .8fr, .8fr, .8fr, .5fr, .8fr, .8fr),
      table.header([Weapon], [Type], [Damage], [STR/DEX], [ENC], [Size], [Cost]),
      [Unarmed], [-], [1D3], [-/-], [-], [-], [-],
...
#figure(
  ...
  text(size: 7.5pt)[
    #table(
      columns: (1.1fr, .8fr, .8fr, .8fr, .8fr, .5fr, .8fr),
      table.header([Ranged Weapon], [Type], [Damage], [Range], [STR/DEX], [ENC], [Cost]),
...
#figure(
  ...
  table(
    ...
    table.header([Armour], [AP], [ENC], [Cost]),
    [Leather], [2], [3], [500 SP],
...
+ Each combatant rolls 1D10 + Combat Order. Higher acts earlier.
+ On their turn, each combatant may take one action and one movement.
+ Each combatant may take one reaction during the round.
+ When all turns are complete, start a new round if the fight continues.
...
+ Attacker rolls the attack skill.
+ If the attack fails, it misses.
+ If the attack succeeds and the defender has a reaction, the defender may Dodge or Parry.
+ Resolve attack and defence.
+ If damage lands, roll weapon damage + Damage Modifier, then subtract armour AP.
+ Apply remaining damage to HP.

#figure(
  caption: "Attack Resolution",
  text(size: 8pt)[
    #table(
      columns: (auto, 1fr, 1.5fr),
      table.header([Attacker], [Defender], [Result]),
...
#figure(
  caption: "Major Wound Effects",
  text(size: 8pt)[
    #table(
      columns: (auto, 1fr),
      table.header([D10], [Injury]),
...
#figure(
  caption: "Social Target State",
  table(
    columns: (1fr, auto),
    table.header([Target State], [Target Modifier]),
...
#figure(
  caption: "Talent Costs",
  table(
    columns: (auto, auto, 1fr),
    table.header([Talent Scale], [Cost], [Guideline]),
