...
\begin{description}
	...
	\begin{rpg-list}
	\item Determine Order: At the start of every combat, calculate the Combat Order or each character involved, which will determine the order in which they will act for the round. To calculate Combat Order add DEX and INT and divide by 2, then add 1D6 for some variability.
	%then subtract half your Armor's ENC and finally add 1D6 for some variability.
	\item Characters Take Actions \& Reactions: In a combat round each character gets one Combat Action, one Movement Action and one or more Defensive Reactions. Combat Actions, such as attacks, and Movement Actions take place in Combat Order. The character with the highest order will act first, followed by the character with the second-highest order, and so on until the character with the lowest order acts. Reactions, such as parries or dodges, are made during this process as they are needed.
...
For example: Lura has DEX 15 and INT 14 and rolled a 3 for a total of 18 Combat Order. A Goblin has DEX 13 and INT 10 and rolled a 4 for a total of 16. Thus the Combat Order is Lura acting before the Goblin.
\end{rpg-examplebox}
...
\item You get one Combat Action, usually an attack, and one Movement Action. You can have more than one Defensive Reaction, usually a defensive action, per combat round.
