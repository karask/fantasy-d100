...
\begin{description}
	...
	\begin{rpg-list}
	\item Determine Order: At the start of every combat each character rolls 1D10 and adds it to their Combat Order. The higher the result the earliest the character acts. In ties the character with the higher DEX acts first.
	\item Characters Take Actions \& Reaction: In a combat round each character gets one Combat Action, one Movement Action and one Reaction. Combat Actions, such as attacks, and Movement Actions take place in Combat Order. The character with the highest order will act first, followed by the character with the second-highest order, and so on until the character with the lowest order acts. Reactions, such as parries, dodges or opportunity attacks, are made during this process as they are needed.
...
\begin{rpg-examplebox}
  For example: Lura has DEX 15 and INT 14. She is wearing leather armour so has Combat Order 12 (15 minus 3). She rolls a 7 in her 1D10 for a total of 19 Combat Order. An unarmoured Goblin has DEX 13 and INT 10 and rolled a 4 for a total of 16. Thus the Combat Order is Lura acting before the Goblin.
\end{rpg-examplebox}
...
\begin{description}
	...
	\item[Close Combat Attack:]  The character can make a single close combat attack. In addition to the normal attack, there are the following special attacks.
		\begin{rpg-list}
		\item All out Attack: The attacker gives up their Reaction for the round but gains a second attack, which happens straight after the first attack. Both attacks are rolled with a Penalty die (-1P) due to the loss of skill during this frenzied attack. This type of attack cannot be combined with any other special attack.
		\item Great Attack: This attack is made using swords, axes or maces where the attacker has enough room to wind up the weapon for a really forceful blow. The attacker rolls with a Bonus die (+1B) to attack and automatically does the maximum damage modifier value but loses his reaction for that combat round.
		\item Disarming Attack: Attacker rolls with a Penalty die (-1P) to his weapon skill with the aim of disarming their opponent either of their weapon or shield. If the attack is successful and the opponent fails to parry or dodge, the weapon or shield is thrown 1D6 metres away from the owner. Larger opponents will have a Bonus die (+1B) or more to their skill roll for this test.
		\item Trip Attack: Attacker rolls with a Penalty die (-1P) to his weapon skill with the aim of tripping their opponent. If the attack is successful and the opponent fails to parry or dodge, he falls prone. Quadruped opponents or larger opponents will have a Bonus die (+1B) or more to their roll for this test.
    \item Knockout Attack: Attacker rolls with a Penalty die (-1P) to his weapon skill with the aim of knocking out their opponent. If the attack is successful and the opponent fails to parry or dodge, and the damage done is equal to a major wound (after armour) then the opponent is knocked out without suffering any damage. If the damage is equal to a minor wound the opponent takes the minimum damage of the weapon but is not knocked out.
		\item Knock-Back Attack: Requires a successful shield attack (with a Penalty die (-1P) on the roll if in off-hand). If the opponent fails to parry or dodge he takes no damage but is knocked-back 2 metres. Larger opponents will have a Bonus die (+1B) or more to their roll. If the opponent fails they need to make an Athletics roll with a Penalty die (-1P) or also fall prone.
		\end{rpg-list}
	\item[Intimidate/Persuade:]  The character tries to get the other side to surrender or flee. This can either be targeted at a single enemy or a group.  Do an Opposed roll using the character’s Influence vs. the enemies’ Persistence, applying the roll as listed below. Groups roll once using the Persistence of the group leader. If the group leader’s Influence skill is higher than his Persistence, then they may use that skill instead. Apply the following Bonus/Penalty dice to the enemy’s skill rolls depending on the state of the enemy.
	\begin{rpg-table}[|c|X|]
		\hline
		two Bonus dice (+2B) & if the enemy is still at full strength, or has only taken some minor wounds.\\
		Bonus die (+1B) & if the enemy out numbers the player’s side, but have had at least 20\% losses either in numbers or hit points.\\
		Penalty die (-1P) & if the enemy is fewer than the player’s side and has taken some wounds.\\
		two Penalty dice (-2P) & if the enemy has taken more than half hit points in wounds and/or has seen half his group incapacitated by the players.\\
		\hline
...
If he decides to single out the wounded Goblin, then the Goblin’s Persistence roll to resist Rurik’s taunting and the resultant urge to flee will be rolled with a Penalty die (-1P). If he decides to target the whole group, which as a whole is undamaged and outnumbers him, then the Goblins will roll with a Bonus die (+1B) to their Persistence. 
	\end{rpg-examplebox}
...
\begin{table}
\begin{center}
\caption{Close Combat Situational Bonus/Penalty Dice}
...
\begin{rpg-table}[|X|c|]
        ...
        Target is prone or attacked from behind (flanked) & +1B\\
	Attacking or defending while on higher ground or on mount (unless wielding a weapon two size categories smaller) & +1B\\
        Attacking or defending while prone & -1P\\
        Attacking or defending while on unstable ground & -1P\\
        Attacking or defending while underwater & -2P\\
	Defending while on lower ground or against mounted foe (unless wielding a weapon two size categories larger) & -1P\\
        Fighting in partial darkness & -1P\\
        Fighting in darkness & -2P\\
        Fighting in pitch dark or blinded & -3P\\
        \hline
\end{rpg-table}
\end{center}
\end{table}
...
\begin{rpg-table}[|l|l|X|]
        ...
        Critical & Fumble          & Attacker does two of: maximum damage, ignore armour, trip, disarm, knock-back, knockout, free attack. Defender fumbles.\\
        Critical & Failure         & Attacker does two of: maximum damage, ignore armour, trip, disarm, knock-back, knockout, free attack.\\ 
...
The difference in SIZ between characters matters considerably. %The Gamemaster discression is important here but as a general guideline the larger character gets a +1B to their grapple check for every 7 points of SIZ difference they have from their opponent.
...
\label{tab:grappling-modifiers}
	\begin{rpg-table}[|X|c|]
	\hline
        \textbf{Situation} & \textbf{Skill Roll}\\
	\hline
        Attacker is $1/3$ larger in SIZ   & Bonus die (+1B)\\
        Attacker is $1/2$ larger in SIZ   & two Bonus dice (+2B)\\
...
\item[Range:] A target within the weapon’s range may be attacked without penalty. A target within double the weapon’s range may be attacked, but the attacker’s weapon skill is rolled with two Penalty dice (-2P). Attacks cannot be made at a distance beyond twice/double the weapon’s range.
\item[Parrying:] The target may attempt to Parry a hand thrown ranged attack with a Penalty die (-1P) but may not normally Parry ranged missile weapons (such as Bows and Crossbow fire). Shield-carrying characters may Parry hand thrown missile weapons with no penalty.
\item[Dodging:] The target may attempt to Dodge a ranged attack with a Penalty die (-1P).
...
\item[Damage Modifier:] A character may add their Damage modifier to ranged weapons except for crossbows.
...
\begin{description}
	\item[Partial cover (Penalty die -1P):] For example a low wall that protects the legs leaving only head and torso exposed.
	\item[Very good cover (two Penalty dice -2P):] For example a defender on a castle wall, firing from protected battlements.
	\item[Virtual total cover (Impossible / GMs discretion):] For example a castle wall with arrow slits for defenders to shot through.
\end{description}
...
\label{tab:ranged-attack-situational-modifiers}
	...
	\hline
        Target has moved 10m or more since attacker's last Combat Action  & -1P\\
        Target has moved 30m or more since attacker's last Combat Action  & -2P\\
	\hline
	\multicolumn{2}{|p{\linewidth-4.5mm}|}{Target Visibility$^{1}$}\\
	\hline
        Target obscured by smoke, mist or is in partial darkness          & -1P\\
        Target obscured by thick smoke, fog or is in darkness             & -2P\\
        Target is above SIZ 20                                            & +1B\\
	\hline
	...
	\hline
        Attacker is prone                                                 & -2P\\
	      Attacker is underwater$^{3,4}$                                    & -1P\\
	      Attacker is on unstable ground                                    & -1P\\
	      Attacker is blinded$^{4}$                                         & -3P\\
        Attacker is engaged in melee$^{5}$                                & -2P\\
	\hline
	\multicolumn{2}{|p{\linewidth-4.5mm}|}{1. Modifiers within these sections are not cumulative. However, modifiers from different sections are cumulative. Therefore, shooting at a target within a mist that has moved more than 10m since the attacker’s last Combat Action imparts two Penalty dice (-2P).}\\
	...
	\multicolumn{2}{|p{\linewidth-4.5mm}|}{5.~Per enemy; i.e. if engaged with two enemies the modifier will be -4P.}\\
	\cline{1-2}
\end{rpg-table}
\end{center}
\end{table}
...
\item[Fighting Retreat:] A character may move a distance up to their full Movement directly away from an enemy he is fighting. He may only defend at +1B (+2B with a Medium and Large shield).
...
\begin{description}
	\item[Parries] can be made against close combat attacks. A character can also parry hand thrown missile weapons at a -1P or without penalty if using a shield.% Shields with a size of Large or Huge (i.e. Medium and Large Shields) provide a cover modifier to the ranged attack of the attacker of -1P and -2P respectively against arrows, sling shot and crossbow bolts. 
	\item[Dodges] can be made normally against close combat attacks and at a -1P penalty for ranged attacks providing the target is aware of the attack.  
	\item[Opportunity Attack] can be made when a character disengages from close combat. At the GM's discretion it can apply to other contested situations as well.
\end{description}
...
%\noindent Characters can make several reactions, but they get a cumulative -20\% modifier for every reaction after the first.
...
%Every attack skill a character possesses has a critical score. A critical score is the tens value of the modified attack skill’s score. It represents a particularly effective hit that causes more than normal damage to an opponent.
...
In close combat a critical hit can cause two of the following effects: do maximum damage (including the damage modifier, unless it is negative), ignore armour, free attack, trip, disarm, knock-back or knockout.

\begin{rpg-examplebox}
 Rurik with his 55\% Close Combat, rolls a 05, which is a critical! He chooses to do maximum damange and ignore armour. Rurik is wielding a Longsword with a damage of 1D8 and has a damage modifier of 1D6. He is fighting a heavily armoured Knight, who has the latest Plate Mail armour (AP 6). However this Armour is completely ignored as Rurik’s sword slides through a gap in the plates doing a devastating 14 points of damage (8 from the sword and 6 from the damage modifier).
\end{rpg-examplebox}

In ranged combat a critical hit will do maximum damage (including the damage modifier, unless it is negative) and ignore armour.

When parrying a successful attack with a critical parry you can block all damage regardless of the weapon size difference.
...
\begin{rpg-list}
	...
	\item Weapon is damaged or breaks.
\end{rpg-list}
...
They must immediately make a Resilience roll, with two Penalty dice (-2P), or fall unconscious. If the test is successfull then the character’s DEX is immediately halved and the character may only fight on for as many combat rounds as their remaining hit points before falling unconscious.
...
\begin{rpg-table}[|c|X|]
        ...
        \hline
        1  & Lose an eye, all Perception rolls suffer two Penalty dice (-2P), lose 4 points of DEX, 1 point of CHA permanently.\\
        2  & Cracked skull, brain damage. Lose 4 points of INT. All skills involving mental processes suffer a Penalty die (-1P) permanently. This includes Perception, Persistence, and all Lore, Culture, and Craft skills.\\
        ...
        5  & Broken ribs, all skills suffer two Penalty dice (-2P), due to severe pain. Lose 2 points of DEX permanently.\\
...
\item Gain a single bonus Close Combat Attack action. This bonus attack is at a Penalty die (-1P) to Close Combat Skill and may only be a normal Close Combat Attack, not a special attack like All Out Attack, Disarming Attack, Great Attack, etc.
%\item Gain a single bonus Close Combat Attack action. This bonus attack is at a Penalty die (-1P) to Close Combat Skill. The second attack occurs at half the character’s combat order. Also this may only be a normal Close Combat Attack, not a special attack like All Out Attack, Disarming Attack, Great Attack, etc.
\end{rpg-list}

% TO ADD if enough space!
%\begin{rpg-examplebox}
%	A warrior armed with sword and shield, can attack with the sword normally at combat order 15 and then follow this up at combat order 8 with a shield bash at a Penalty die (-1P) to the shield attack.
%\end{rpg-examplebox}
...
Instead, an opposed skill check may be rolled to determine the winner of the \emph{contest} immediately. See page~\pageref{ssec:opposed-tests-over-100} for more on opposed skill tests of over 100\%.
...
%\begin{rpg-examplebox}
%For example Curgan the Mighty with an Close Combat skill of 150\% can split it 75\% / 75\% or make three attacks at three opponents in range at 50\% each.
%\end{rpg-examplebox}
...
\begin{rpg-table}[|l|X|]
        ...
        All Out Attack       & Two attacks at a Penalty die (-1P). Gives up Reaction for round.\\
        Great Attack         & One attack at a Bonus die (+1B) with maximum damage modifier. Gives up Reaction for round.\\
        Disarming Attack     & Attack at a Penalty die (-1P) to disarm opponent. Weapon or shield is thrown 1D6 meters away.\\
        Trip Attack          & Attack at a Penalty die (-1P) to trip opponent. Opponent falls prone.\\
        Knockout Attack      & Attack at a Penalty die (-1P) to knock out opponent. If the damage is equal to a major wound the target is knocked out. Otherwise minimum weapon damage.\\
	Knock-Back Attack    & Attack with shield (at a Penalty die (-1P) if in off-hand). If opponent fails to parry or dodge he is pushed back 2 meters and needs an Athletics roll at a Penalty die (-1P) or fall prone.\\
        Set Weapon           & As an action the character sets a spear or polearm in anticipation of a charge. When it occurs the character attacks first with a Bonus die (+1B) to weapon skill.\\
	...
	Fighting Retreat     & Moves their Movement Rate and defends at a Bonus die (+1B) (+2B if using a Medium or Large Shield).\\
	Sprint               & Moves twice Movement Rate. May not attack and may only Dodge as a reaction.\\
	Ranged Combat Attack & Character attacks opponent with weapon, tests vs. Ranged Combat skill if successful then does weapon damage (plus damage modifier, except for crossbows).\\
	Ranged Combat Aim    & Character aims for a combat round to gain a Bonus die (+1B) to the next ranged attack.\\
