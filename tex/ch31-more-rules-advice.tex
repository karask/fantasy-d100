...
\begin{rpg-table}[|X|Y|]
        \hline
	25\% or less (no idea) & Penalty die (-1P)\\
	26\%-50\% (competent) & -\\
	51\%-75\% (veteran) & Bonus die (+1B)\\
	76\% or more (expert) & two Bonus dice (+2B)\\
	\hline
\end{rpg-table}
\end{center}
\end{table}
...
%Independent of whether the Persistence test was successful, they must immediately make a Resilience test with two Penalty dice (-2P), or go into shock for 1D4 rounds. 
...
\begin{rpg-list}
\item Better equipped than enemy: Bonus die (+1B).
\item Better trained than enemy: Bonus die (+1B).
\item Has significant special forces (e.g. artillery, cavalry, combat mages) that the enemy lacks: Bonus die (+1B) for each.
\item Outnumber enemy by two to one or more: Bonus die (+1B).
\item Outnumber enemy by four to one or more: two Bonus dice (+2B).
\item Enemy in defensive position: Penalty die (-1P).
\item Enemy fortifications: two Penalty dice (-2P).
\item Player character heroics (eg: taking out significant enemy, capturing strategic position): Bonus die (+1B), or Penalty die (-1P) if attempted heroics go horribly wrong.
\end{rpg-list}
...
To save time and keep the game flow a GM may consider a Group test. This is appropriate in cases were all the players need to roll and any one of them failing makes the whole group fail. For example, if the group is rolling for Deception to stealtily move through an area you can have only the player with the lowest Deception roll.

Similarly, if 8 guards all have to roll Perception to spot the characters, the GM may judge that rolling only for the guard with the highest Perception (maybe with a bonus, since the other guards aid him) is enough to determine the result.

\subsection{Sorting Tests}
Another type of time-saving techique may apply when the GM needs to sort out which NPCs succeed with something or not according to their skill. For example, several NPCs try to jump from rooftop to rooftop. The GM may roll only once and apply this roll to all the NPCs according to their Athletics skill. 
...
\subsubsection{Rules}
The Player can, of course, use the Battle discipline to improve his fighting abilities but to emulate some of the special monk abilities we can use the Folk Magic discipline. The Player needs to pay Improvement Points to get the Folk Magic discipline and then pay IPs for each Magnitude of spells he is learning. The rules and maximum Magnitude they can learn is exactly as the Folk Magic discipline. The Player character will not be casting spells but all the effects of spell casting need to be emulated. For example it should be obvious for anyone that is seeing the Monk that he is initiating something and under duress they may not succeed.
...
Note, however, that they can only be applied to `self' and never to others; for this reason the Gamemaster may eschew the Folk Magic Discipline IPs or otherwise balance the spell limitation.
...
\subsection{Rules}
To enable such mechanics Gamemasters might allow certain Folk Magic spells to be cast as a spell-like ability once per day, or more, and activated with a Combat Action. This is similar to how permanent charms are created with the Create Charms spell.

The player spends three Improvement Points per Magnitude to enable them to use the spell as an ability once per day (1/day); without requiring a Folk Magic Casting skill or any Power. It can be activated as an Action. 
...
Also note how her magical amulet containing a Protection 2 Folk Magic spell is described.

Finally, extraordinary and supernatural abilities can be described with a brief Discipline abbreviation and then the appropriate Powers. For Disciplines, B is for Battle, FM is for Folk Magic, AM for Arcane Magic, DM for Divine Magic, etc.

\vspace{1em}
\begin{rpg-examplebox}
Goblin Witch Doctor 60 (Dagger 1D4+1; FM: Heal, Scare, Sap Energy)
\end{rpg-examplebox}

The goblin witch doctor could have a 60\% at his Healing Skill and a 50\% in his Folk Magic Casting skill. He probably has a 40\% in Persistence, Resilience and Dodge and knows 3 Folk Magic spells: Heal, Scare and Sap Energy.
...
\begin{figure}[H]
\begin{center}
\includegraphics[scale=0.38]{img/ai-images/thief.png}
\end{center}
\end{figure}
