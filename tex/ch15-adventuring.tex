...
\begin{rpg-table}[|c|X|X|]
	...
	Partial Darkness     & Cavern mouth, misty day, within 3 x radius of illuminating item (see below). & -1P to vision-based Perception tests and combat attacks.\\
	Darkness             & Large cavern illuminated only by embers, foggy day, within 5 x radius of illuminating item. & -2P to vision-based Perception tests and combat attacks. Movement rate halved.\\
	Pitch Black          & Sealed room with stone walls, cavern many miles underground, supernatural darkness, mountaintop whiteout. & Perception tests reliant on vision become near impossible, as are ranged attacks. Close combat attacks suffer three Penalty dice (-3P). Movement rate a quarter of normal.\\
        \hline
\end{rpg-table}
\end{center}
\end{table*}
...
If a character fails the Resilience test then they become fatigued. All skill tests suffer a Penalty die. Also movement rate drops by a quarter. The character also becomes sluggish, DEX and INT are each reduced by three points for the purposes of determining Combat Order.

If the fatigued character insists on engaging in heavy activity, such as combat, heavy labour or running, then another Resilience roll is made with a Penalty die (-1P). If the character fails this second skill test they become heavily fatigued and all the above penalties are doubled (two Penalty dice (-2P)).
...
A character can survive for a number of days equal to his CON before becoming starved, though after three days they will begin to suffer a Penalty die (-1P) to Fatigue tests. 
...
Whenever a character is suffering from exposure, starvation or thirst, a Fatigue test is required with a Penalty die (-1P). In addition, the character will automatically suffer one 1D6 of damage every day, for every condition he is experiencing. Natural healing will not heal this damage – only sufficient shelter, food or water can remedy the problem and allow natural healing to take place. 
...
\begin{rpg-list}
\item Overloaded characters suffer a Penalty die (-1P) to all tests that require physical actions, including Weapon skill tests and most tests that have DEX or STR as a Characteristic. 

\item Overloaded characters have their Movement halved. They also suffer a -1P to all Fatigue tests. 
\end{rpg-list}
...
Once a character has surpassed the time for which he can hold his breath, he must make a Resilience test every round with a cumulative Penalty die (-1P). If he fails, he automatically starts inhaling the suffocating substance.
