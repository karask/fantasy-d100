...
Disruption literally pulls a target’s body apart. The target will suffer 1D4 points of damage per point of Magnitude, ignoring any Armor Points. 
\end{rpg-spell}
...
With this spell, the caster throws a stream of fire at his target. If the fire is not dodged, it inflicts 1D10 points of heat damage. Armor Points are effective against this damage and it counts as both magical and fire damage.
\end{rpg-spell}
...
This attack spell allows the caster to freeze their opponent, dealing 1D8 points of damage, ignoring any Armor Points. Magical protection against cold damage can block this effect, but mundane items (such as cold weather gear) are ineffective.
\end{rpg-spell}
...
This fearsome spell allows the caster to deal an awful wound with the merest touch. Casting the Hand of Death, charges his body with the spell. Touching an unsuspecting target, or succeeding at an Unarmed attack against a wary target, releases the spell’s effect. If the Resilience test to resist the effect is failed, the victim immediately loses half their maximum Hit Points, and suffers a a Major Wound. If the Resilience test is a success, the target only loses 1D3 Hit Points. Armor does not protect against this damage.
\end{rpg-spell}
...
This spell can be cast on any weapon with a blade or point. For every point of Magnitude, it ignores one armor point when it strikes armor. Pierce can bypass magical armor as easily as normal armor. 
\end{rpg-spell}
...
%For every two points of Magnitude of this spell one armor point is added to the armor of the target. This stacks with any existing armor and is treated in the same way. 
For every point of Magnitude of this spell one armor point is added to the armor of the target. This stacks with any existing armor and is treated in the same way. 
\end{rpg-spell}
...
The caster summons a lightning bolt from the heavens regardless of the weather. The target must be outdoors in plain view. Skybolt inflicts 2D6 points of damage to a single chosen target. Only magical Armor Points offer protection against this damage.
\end{rpg-spell}
...
\begin{description}
\item [Create Charms] This is the basic spell for creating magic items. Use this spell to create rune-inscribed swords, paper talismans that protect against spirits, and dragon skin armor that is resistant to fire (via a Resist Fire spell).
...
\begin{figure}[h]
\begin{center}
\includegraphics[scale=0.24]{img/ai-images/tower.png}
\end{center}
\end{figure}
