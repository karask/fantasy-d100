...
\subsection{Limits to Folk Magic}
...
The result of the Folk Magic casting test depends on its success:
\begin{description}
...
%\begin{rpg-list}
%\item POW has no limit.
%\item Great Spirits are more adept at dealing with the Spirit World and understand the weaknesse%s of their spiritual foes. Thus spirit damage is increased to 1D6+2 Power Points.
...
The caster whispers into the ear of a distressed animal, calming it. If the distressed animal is under the influence of a spell such as Fear or Scare, then it gets another Persistence test to shake off the effect of the spell.
\end{rpg-spell}
...
\begin{rpg-spell}
{Awareness} %Back Eyes and Cover Blind Side !!
...
This spell confuses and clouds the mind of its target if they fail a Persistence roll. The affected target may not cast spells and may only take non-offensive actions. The target may run if it so chooses and may dodge and normally parry in combat. Any skills that have INT as a base are at a Penalty die (-1P) when tested while the target is under the effects of this spell.
...
For every two points of Magnitude, the target’s combat order increases by +4, whether casting spells or fighting. In addtion the target gains a Bonus die (+1B) to Dodge or DEX-based Athletics tests.
\end{rpg-spell}
...
Like Detect (X), this includes a number of different spells, each of which affects a different non-combat skill. For every 2 points of Magnitude, the recipient gains a Bonus die (+1B) to any skill test using the Enhanced skill.  Alternatively, for each additional 2 points of Magnitude of the spell, the caster can affect one more target. The bonuses and targets can be split as necessary, providing each bonus is a Bonus die (+1B) (costing 2 Magnitude).

For example, Adjin may have Enhance Skill (Deception) 4.  He could cast it all on himself to give two Bonus dice (+2B) to his Deception, or could cast it on himself and an ally, giving himself a Bonus die (+1B) and his ally a Bonus die (+1B).
...
The target of this spell will have close combat and unarmed combat skills increased by a Bonus die (+1B) but may not attempt to parry, dodge or cast spells. Also for the duration of the spell the target has two Bonus dice (+2B) bonus to any Persistence test related to Morale. The effects of this spell are automatically cancelled by the Demoralise spell and vice versa.
\end{rpg-spell}
...
Like Enhance Skill (X), this is a number of different spells, each of which affects a different skill. For every 2 points of Magnitude of the spell, the target gains a Penalty die (-1P) to the next skill test using the affected skill.
...
This spell hardens the resolve of the character that it is cast upon for its duration. Each 2 levels of Magnitude of the spell adds a Bonus die (+1B) to all Persistence tests against magical attacks to the mind (e.g. Fear, Befuddle etc.) or opposed tests vs Influence.
\end{rpg-spell}
...
This spell adds a resistance to being opened to a lock equal to the spell’s Magnitude x 10\%. The item must have a lock, such as might be found on a door or a chest, and the spell is focused on that lock. Once the lock has been forced/picked the spell ends.
\end{rpg-spell}
...
This spell adds a Bonus die (+1B) to Resistance tests against hostile effects, magic or otherwise, from a given element (Air/Darkness/Earth/Fire/Water) per 2 Magnitude, and subtracts 2 points of damage from that element per Magnitude.
\end{rpg-spell}
...
{Magnitude 1, Non-Variable, Resist (Dodge)}
  ...
  The caster makes the ground under the target’s feet as slippery as a sheet of black ice. The target must make a Resist (Dodge) roll or fall over prone.
\end{rpg-spell}
...
Cast on a missile this spell is triggered when it is fired. It gives a Bonus die (+1B) to Ranged Combat and +3 damage while using the missile. A missile under the effects of Speedart cannot benefit from Fire Missile or Multi Missile.
\end{rpg-spell}
...
For every two points of Magnitude this spell increases the spiritual damage a character causes during Spirit Combat by one step (so a 1D4 becomes a D6, a D6 becomes a D8 and so forth), Persistence is also increased by a Bonus die (+1B) per two points of Magnitude.
\end{rpg-spell}
...
For every two points of Magnitude of this spell, the target’s Damage increases by +2 and strength based athletics tests gain a Bonus die (+1B). Note the Damage increase is not treated as magical damage.
\end{rpg-spell}
...
This spell grants the caster a thunderous voice of command. For every 2 points of Magnitude of this spell, the caster has a Bonus die (+1B) added to his Influence skill and can also be heard at up to the spell’s Magnitude x 100 in metres.
\end{rpg-spell}
...
This spell grants the caster a Bonus die (+1B) per 2 points of Magnitude to their next Skill Test they make to discover lies, secrets or hidden objects. It does not stack with any other spell-effect bonuses.
\end{rpg-spell}
...
This spell has a chance of opening a lock equal to the spell’s Magnitude x 20\%. If cast on a lock that has had a Lock spell cast on it, the test is an Opposed Test vs the Magnitude x 20\% of the Lock spell.
\end{rpg-spell}
...
\item Player characters use either Persistence skill. %or Magic Casting if they are Shamans. Shamans can use Persistence if they so choose.
...
% END - AAAAAAAA NEEDS TO GO SOMEWHERE AAAAAAAA
...
There is no catch all “Detect Magical Properties” or “Know Magic item” skill in Fantasy Crux. This is quite deliberate, keeping with the general policy that such items are not the equivalent of magical shotguns.  Some options are:
...
\begin{figure}[h]
\begin{center}
\includegraphics[scale=0.3]{img/ai-images/tower.png}
\end{center}
\end{figure}
