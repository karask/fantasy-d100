...
\begin{description}
	...
	\item[STR/DEX:] The minimum STR and DEX scores needed to easily wield this weapon. If either of the Characteristics are below these minimums, a Penalty die (-1P) is applied to a character’s skill when attacking and parrying with this weapon. 
...
\begin{description}
	...
	\item[Damage Dice:] The damage the weapon deals on a successful attack. 
	\item[Range:] This is the effective range of the weapon. A target within the weapon’s range may be attacked without penalty. A target within double the weapon’s range may be attacked, but the attacker rolls with two Penalty dice (-2P). Attacks against targets beyond double the weapon’s range automatically fail.
	\item[STR/DEX:] The minimum STR and DEX scores required to easily wield this weapon. If either of the Characteristics are below these minimums, a Penalty die (-1P) is applied to a character’s roll when attacking and parrying with this weapon.
	\item[ENC:] The weapon’s Encumbrance. The weight and bulk of the weapon. 
	\item[Cost:] The cost in silver pieces to purchase this weapon.
\end{description}
...
\begin{rpg-table}[|X|c|c|c|c|c|c|]
	...
	\hline
	Dagger            & Close     & 1D4+1 & STR x m  & -/9  & -  & 30 SP\\
  Crossbow (Heavy)$^{1}$ & 2H        & 2D6   & 150m     & 9/9  & 2  & 350 SP\\
  Dart              &           & 1D3   & STR x m  & -/9  & -  & 15 SP\\
  Crossbow (Light)$^{1}$ & 2H        & 1D8   & 125m     & 5/9  & 1  & 150 SP\\
	Hatchet           & Close     & 1D6   & STR x m  & -/9  & 1  & 25 SP\\
	Improvised (Rock) &           & 1d4   & STR x m  & 5/5  & 1  & -\\
	Javelin           &           & 1d6   & STR x 2m & 5/9  & 1  & 20 SP\\
	Longbow           & 2H             & 1d10  & 150m     & 13/9 & 1  & 150 SP\\
	Shortbow          & 2H             & 1d8   & 75m      & 9/9  & 1  & 75 SP\\
	Shortspear        & Close   & 1d6   & STR x 2m & 5/9  & 2  & 20 SP\\
	Sling             & 1H             & 1d6   & 50m      & -/9  & -  & 5 SP\\
	Whip              & Close          & 1d3   & 5m       & -/9  & -  & 50 SP\\
	\hline
  \multicolumn{7}{|p{\linewidth-4.5mm}|}{1. To reload a heavy crossbow requires a full combat round while a light crossbow requires both movement action and reaction (see Combat Chapter for details).}\\
	\hline
\end{rpg-table}
\end{center}
\end{table*}
...
%	\item[Thrown:] A character can use their damage modifier with this weapon.
...
\begin{description}
	...
	\item[Block \& Tackle:] Adds a Bonus die (+1B) to Mechanisms tests to make or disarm large traps and makes Engineering tests possible in some circumstances. It requires at least 10m of rope to function. 
	...
	\item[Climbing Kit:]  A climbing kit provides a Bonus die (+1B) to any Athletics skill tests made to climb. 
	\item[Crowbar:] Adds a Bonus die (+1B) to brute force Athletics tests. If used as a weapon, it is considered a club (wielded with a Penalty die -1P). 
\end{description}
...
\begin{description}
	\item[Fishing Kit:] The fishing kit grants a character a Bonus die (+1B) to his Natural Lore test to catch fish. 
	...
	\item[Hammer:] If used as a weapon, it is treated as a club (wielded with a Penalty die -1P). Hammers may be used on inanimate objects without being destroyed. 
	...
	\item[Lock Picks:] Adds a Bonus die (+1B) to Mechanisms tests to unlock a locking device. 
	\item[Mining Pick:] If used as a weapon, it is considered a club (wielded with a Penalty die -1P). Mining picks may be used on inanimate objects without being destroyed. 
	...
	\item[Scythe:] If used as a weapon, it is considered a polearm (wielded with a Penalty die -1P). 
	\item[Slingbag:] It can carry 15 ENC of equipment. 
	\item[Spade:] If used as a weapon, it is considered a club (wielded with a Penalty die -1P). 
	\item[Torch, 1 Hour:] It will burn for one hour. A torch illuminates a three metre radius. If used as a weapon, it is considered a club (wielded with a Penalty die -1P), except that it does not inflict normal damage – instead, it inflicts 1D4 fire damage and a fumble or critical hit will also extinguish the brand. 
...
\begin{figure}[h]
\begin{center}
\includegraphics[scale=0.43]{img/ai-images/desert-sunset.png}
\end{center}
\end{figure}
