...
On one hand the character is a collection of numbers which describe their characteristics, skills and magic spells that are written down on a character sheet.  This chapter will explain how you create these numbers, in a process known as Character Generation. But that is only half of what a character is.
...
This method is appropriate if you already have a clear idea of your character concept as it gives you precise control on the relative strength of each characteristic. You are not at the mercy of random dice rolls (see Random Characteristics optional rule) nor do you have to negotiate with your Gamemaster about switching the random rolls around so that the characteristic scores match your concept.
...
Players can also roll for characteristics. For human characters roll 4D6 for STR, CON, DEX, POW and CHA and 3D6+6 for INT and SIZ. Discard the lowest die. You may swap one pair.
\end{rpg-titlebox}
...
\begin{description}
  \item[Damage Modifier (DM):] The Damage Modifier applies whenever the character uses a weapoin (except crossbows). It is used to show that bigger, stronger, characters deal out more damage than smaller, weaker characters. To work out your character’s Damage Modifier, add Strength and Size together and look at table~\ref{tab:damage-modifier}.
	\item[Hit Points (HP):] These determine how much damage the character can sustain before reaching unconsciousness or death. Hit Points equal Size plus Constitution divided by 2, rounded up. 
	\item[Major Wound Level:] When the character takes this amount of damage in one go, they suffer a Major Wound. Major Wounds are injuries, such as a severed limb, broken ribcage or broken skull, which may render the character unconscious immediately and leave permanent affects if not healed by supernatural powers. Major Wound Level is equal to Total Hit Points divided by 2, rounded up.
	\item[Power Points (PP):] These are used to activate any extraordinary or supernatural powers the character knows. If it is reduced to zero the character drops unconscious until they regain at least one point. The character’s starting Power Points will be equal to the character’s POW.
	\item[Movement Rate:] This is the amount in metres that a character can move in a five second Combat Round. Human characters have a Movement rate of 15 metres.
	\item[Combat Order:] This is the order in which the character acts during combat. To calculate Combat Order add DEX and INT and divide by 2. Then substract your armour's ENC from the result.
\end{description}
...
%To calculate Combat Order add DEX and INT and divide by 2. Then substract your armour's ENC from the result.
...
\subsubsection{Calculate Skills}
...
\item Reduce up to 10 points from Resistances, Combat or Practical skill categories to gain extra Improvement Points (see page~\pageref{ssec:improvement-points}) at a 5:1 ratio; a total of 6 IPs can be acquired this way. These IPs can only be used to get extra Disciplines/Powers and increase Characteristics. Moreover, the starting limit of characteristics still apply.
\end{rpg-list}
...
Each character begins play with ten Improvement Points (see page~\pageref{ssec:improvement-points}) that allow players to further improve in skills or even acquire new Disciplines. For choices of Disciplines and their Powers consult the chapters in Part II.
...
Through their adventures characters will earn experience, awarded by the Gamemaster as a reward for certain achievements. This experience, called Improvement Points, will allow characters to improve.

\subsubsection{Improving Skills}
A player can choose to spend one Improvement Point to increase one or two skills.
Select one or two skills that are below 51\% to be increased and split 5\% points to those skills. Skills from 51\% to 99\% are increased by only 3\% for each IP and skills at Mastery level, i.e. 100\% or more, require one IP for a 1\% in a single skill.
...
A player can choose to spend Improvement Points to acquire new Disciplines. Consult the appropriate Discipline chapter for details of how many points are required to get the Discipline as well as how much is required to get each Power within it.
...
\begin{figure}[h]
\begin{center}
  \includegraphics[scale=0.26]{img/Longsword.png}
\end{center}
\end{figure}
